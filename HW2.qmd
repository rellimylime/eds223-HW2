...
# Set knitr options for code wrapping
...
test_that("all datasets have matching CRS", {
  expect_equal(st_crs(birds), st_crs(ejscreen))
  expect_equal(st_crs(ineq), st_crs(ejscreen))
  expect_equal(st_crs(birds)$epsg, 3857)
})
```
...
# Perform spatial join such that each block group is counted once
block_grades <- st_join(la_county, ineq, join = st_intersects, largest = TRUE)

# Drop geometries after join
block_grades_df <- st_drop_geometry(block_grades)

# Check that the spatial join worked as expected
test_that("Spatial join completed successfully", {
  expect_equal(nrow(block_grades), nrow(la_county))
  expect_true("grade" %in% colnames(block_grades))
  expect_true(sum(!is.na(block_grades$grade)) > 0)
})
```
...
if(abs(total_percent - 100) > 0.01) {
  warning("WARNING: Percentages sum to ", 
...
if(nrow(grade_summary_plot_graded) != 4) {
  warning("Expected 4 HOLC grades (A, B, C, D), but found ", nrow(grade_summary_plot_graded))
}
```

```{r}
# Plot Low Income vs PM2.5
ggplot(grade_summary_plot_graded, aes(x = mean_lowincome, 
                                      y = mean_part, 
                                      color = grade)) +
...
ggplot(portion_obs, aes(x = grade, y= portion_obs, fill = grade)) +
  geom_col() +
    scale_fill_viridis_d(option = "viridis") +
  labs(title = "Percentage of Bird Observations within HOLC Grades in LA",
       x = "HOLC Grade",
